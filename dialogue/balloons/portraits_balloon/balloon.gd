...
var dialogue_line: DialogueLine:
	set(next_dialogue_line):
		...
		if dialogue_line.responses.size() > 0:
			...
			var time = dialogue_line.dialogue.length() \
				* 0.02 if dialogue_line.time == "auto" else dialogue_line.time.to_float()
...
## Start some dialogue
func start(
		dialogue_resource: DialogueResource, title: String, extra_game_states: Array = []) -> void:
...
func handle_resize() -> void:
	...
	balloon.global_position = Vector2(
		(viewport_size.x - balloon.size.x) * 0.5, viewport_size.y - balloon.size.y)
